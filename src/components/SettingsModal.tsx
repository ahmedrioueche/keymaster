...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  const [language, setLanguage] = useState<string>(
    userSettings?.language
      ? capitalizeFirstLetter(userSettings?.language)
      : capitalizeFirstLetter(defaultLanguage)
  );
  const [typingMode, setTypingMode] = useState<string>(
    userSettings?.mode
      ? capitalizeFirstLetter(userSettings?.mode)
      : capitalizeFirstLetter(defaultMode)
  );
  const [textLength, setTextLength] = useState<number>(
    userSettings?.textLength ? userSettings?.textLength : defaultTextLength
  );
  const [soundEffect, setSoundEffect] = useState<string>(
    userSettings?.soundEffects === true ? "Enabled" : "Disabled"
  );
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="col-span-2 space-y-4 text-light-foreground dark:text-dark-foreground">
            ...
            <div className="flex flex-col">
              ...
              />
              {!isValidTextLength && (
                <span className="text-dark-secondary text-base mt-1">
                  Text length must be between {minTextLength} and{" "}
                  {maxTextLength} letters.
                </span>
              )}
            </div>
