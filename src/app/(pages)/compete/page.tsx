      ...
      <div className={`min-h-screen flex flex-col ${currentUser ? '' : 'p-8'} h-full bg-light-background text-light-foreground transition-all duration-500 dark:bg-dark-background dark:text-dark-foreground`}>
        {!currentUser ? (
          <div className="flex flex-col md:flex-row items-center justify-center mb-8 md:mt-12 md:mb-12">
            ...
            <Image src="/storysets/typing.svg" alt="KeyMaster" className="w-48 h-48 object-contain mr-4" height={128} width={128} />
            ...
            <div className="text-center md:text-left flex flex-col items-center md:items-start">
              ...
              <p className="text-lg md:text-xl font-semibold mb-6">Login to compete against your friends!</p>
          ...
          room && <CompeteRoom room={room} opponent={opponent ? opponent : undefined} currentUser={currentUser} onReady={handleReady} isStarted={isStarted} />
        )}
      </div>
      <CountDown onClose={handleCountdownClose} isOpen={isCountdownOpen} count={countdown} />
      {(!opponent || opponentModalCloseGracefully) && <FindOpponent isOpen={isFindOpponentOpen} onClose={handleFindOpponentClose} onOpponentFound={(opponent, room) => handleOpponentFound(opponent, room)} onCreateRoom={(room) => handleCreateRoom(room)} onJoinRoom={(room) => handleJoinRoom(room)} />}
      <UserModal isOpen={isUserModalOpen} onClose={() => handleUserModalClose()} type="compete" />
    </div>
  );
};
